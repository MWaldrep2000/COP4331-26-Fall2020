<?php
	...
	{
		$sql = "select FirstName,LastName,PhoneNumber,Address,Email,ID from Info where UserID=" . $inData["userId"];
		...
		if ($result->num_rows > 0)
		{
			...
			while($row = $result->fetch_assoc())
			{
				...
				for ($i = 0; $i < 6; $i++)
				{
					...
					{
						case 0:
							$readResults .= '{"FirstName":"' . $row["FirstName"] . '",';
							break;

						case 1:
							$readResults .= '"LastName":"' . $row["LastName"] . '",'; 
							break;

						case 2:
							$readResults .= '"PhoneNumber":"' . $row["PhoneNumber"] . '",';
							break;

						case 3:
							$readResults .= '"Address":"' . $row["Address"] . '",'; 
							break;

						case 4:
							$readResults .= '"Email":"' . $row["Email"] . '",'; 
							break;
						
						case 5:
							$readResults .= '"ContactID":' . $row["ID"] . '}'; 
							break;
	...
	function returnWithInfo( $readResults )
	{
		$retValue = '{"results":[' . $readResults . '],"error":""}';
		sendResultInfoAsJson( $retValue );
	}
